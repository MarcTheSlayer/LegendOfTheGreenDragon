...
/**
	...
	14/10/2013 - v1.0.2
	+ Missing $args['acctid'] from the delete character hook. :(
*/
...
function lodgeneverexpire_dohook($hookname,$args)
{
	...
	switch( $hookname )
	{
		case 'delete_character':
			if( get_module_pref('bought','lodgeneverexpire',$args['acctid']) > '' || get_module_pref('awarded','lodgeneverexpire',$args['acctid']) > '' || get_module_pref('dkreward','lodgeneverexpire',$args['acctid']) > '' ) $args['dodel'] = FALSE;
		break;
		...
		case 'footer-lodge':
			if( get_module_pref('bought') == '' && get_module_pref('awarded') == '' && get_module_pref('dkreward') == '' && !($session['user']['superuser'] & SU_NEVER_EXPIRE) )
			{
				...
				if( get_module_pref('awarded') == '' && $cost > 0 )
				{
					$points = translate_inline(array('point','points'));
					addnav('Use Points');
					addnav(array('Character Never Expires (%s %s)', $cost, ($cost==1?$point[0]:$points[1])),'runmodule.php?module=lodgeneverexpire&op=lodge&sop=buy');
				}
			}
		break;
		...
		case 'pointsdesc':
			$args['count']++;
			$str = translate("Character account that never expires (never auto deleted) costs %s %s.");
	...
	if( $op == 'lodge' )
	{
		...
		elseif( $sop == 'confirm' )
		{
			...
			if( $pointsavailable >= $cost )
			{
				...
				$dconfig = array_push($dconfig, "spent $cost points to buy the NEVER EXPIRE pref.");
				$session['user']['donationconfig'] = $dconfig;
				debuglog('`^has bought the NEVER EXPIRE pref at the lodge for ' . $cost . ' points.');
			}
			else
			{
				output("`n`7J. C. Petersen looks down his nose at you. \"`&I'm sorry, but you do not have the %s %s required. Please return when you do and I'll be happy to do business with you.`7\"", $cost, ($cost==1?$point[0]:$points[1]));
			} 
