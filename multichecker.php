...
/*
	...
	27/02/10 - v1.0c
	+ Changed access so only SUs with user edit powers or those with the 'cansee' pref.
*/
function multichecker_getmoduleinfo()
{
	$info = array(
		...
		"prefs"=>array(
			"cansee"=>"Can this person use the multichecker?,bool"
		)
	);
	return $info;
}
...
function multichecker_dohook($hookname, $args)
{
	...
	if( $session['user']['superuser'] & SU_EDIT_USERS || get_module_pref('cansee') == 1 )
	{
		addnav('Mechanics');
...
function multichecker_run()
{
	...
	switch( $op )
	{
		...
		case 'ID':
			...
			output("A bug in LotGD v1.1.1 and early meant that the registration date was deleted with a DK. If no date appears then that's why.`n`n");
	...
	if( $result = db_query($sql) )
	{
		...
		$name = translate_inline('Login/Char Name');
		...
		foreach( $array as $key => $value )
		{
				...
				foreach( $value as $key2 )
				{
					...
					if( !in_array($key2['login'],$array2) )
					{
						...
						rawoutput("<td><a href='bio.php?char=" . $key2['acctid'] . "&ret=" . urlencode($_SERVER['REQUEST_URI']) . "'>" . $key2['login'] . "</a><br />" . appoencode($key2['name']) . "</td><td align=\"center\">" . $clanid . "</td><td align=\"center\" nowrap=\"nowrap\">" . $time . "</td><td align=\"center\">" . $key2['acctid'] . "</td><td>" . $key2['email'] . "</td><td align=\"center\">" . $referrer . "</td><td align=\"center\">" . $regdate1 . "</td></tr>");
