...
/**
	...
	06/11/09 - v0.0.4
	+ Online count on nav link.
	+ New player count on nav link.
*/
function grotto_warriorsonline_getmoduleinfo()
{
	$info = array(
		...
		"settings"=>array(
			...
			"newplayers"=>"Show new player count for last:,datelength|5 days",
...
function grotto_warriorsonline_dohook($hookname,$args)
{
	...
	addnav('Actions');
	addnav(array('Warriors Online (%s)',($count>0?$count:0)),'runmodule.php?module=grotto_warriorsonline',FALSE,TRUE,'600x400');
	...
	if( $session['user']['superuser'] & SU_EDIT_USERS )
	{
		$sql = "SELECT acctid
				FROM " . db_prefix('accounts') . "
				WHERE locked = 0
					AND regdate > '".date("Y-m-d H:i:s",strtotime("-".get_module_setting('newplayers'), time()))."'";
		$result = db_query($sql);
		$count = db_num_rows($result);
		addnav(array('Warriors New (%s)',($count>0?$count:0)),'runmodule.php?module=grotto_warriorsonline&subop=new&count='.$count,FALSE,TRUE,'600x400');
	}
...
function grotto_warriorsonline_run()
{
	...
	if( $op == 'bio' )
	{
		...
		$sql = "SELECT login, name, level, sex, title, specialty, hashorse, acctid, resurrections, bio, dragonkills, race, clanname, clanshort, clanrank, " . db_prefix('accounts') . ".clanid, laston, loggedin
				FROM " . db_prefix('accounts') . "
					...
					ON " . db_prefix('accounts') . ".clanid = " . db_prefix('clans') . ".clanid
				WHERE acctid = '$userid'";
		$result = db_query($sql);	
	...
	else
	{
		...
		output_notl("`n`c`b(%s %s)`b`c", $count, ($count==1?$warr[0]:$warr[1]));
		if( $subop == 'new' )
		{
			$count = httpget('count');
			output('`c`b(%s in the last %s)`b`c`n`n', ($count>0?$count:0), get_module_setting('newplayers'));
		}
		else
		{
			output_notl('`n`n');
